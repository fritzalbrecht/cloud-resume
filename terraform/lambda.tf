data "archive_file" "postVisitors-lambda" {
  type        = "zip"
  source_file = "${path.module}/lambda/postVisitors/lambda_function.py"
  output_path = "${path.module}/lambda/postVisitors/postVisitors_function_payload.zip"
}

data "archive_file" "getVisitors-lambda" {
  type        = "zip"
  source_file = "${path.module}/lambda/getVisitors/lambda_function.py"
  output_path = "${path.module}/lambda/getVisitors/getVisitors_function_payload.zip"
}

resource "aws_lambda_function" "post_visitors_lambda_terraform" {
  filename      = "${path.module}/lambda/postVisitors/postVisitors_function_payload.zip"
...
resource "aws_lambda_function" "get_visitors_lambda_terraform" {
  filename      = "${path.module}/lambda/getVisitors/getVisitors_function_payload.zip"
